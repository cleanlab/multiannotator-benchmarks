...
def cross_val_predict_autogluon_image_dataset(
    ...
    *,
    ...
    for split_num, split in enumerate(skf_splits):
        ...
        model_pred_labels = predictor.predict(test_data)
        model_accuracy = np.mean(model_pred_labels == true_labels_test)
        model_accuracy_arr.append(model_accuracy)

        pred_probs_test = predictor.predict_proba(test_data).to_numpy()
        pred_probs_test_arr.append(pred_probs_test)

        # get pred_probs on unlabeled data
        if unlabeled_data is not None:
            pred_probs_unlabeled = predictor.predict_proba(unlabeled_data).to_numpy()
            pred_probs_unlabeled_arr.append(pred_probs_unlabeled)
        ...
        avg_model_accuacy = np.mean(np.array(model_accuracy_arr))
        avg_pred_probs_unlabeled = np.mean(np.array(pred_probs_unlabeled_arr), axis=0)
        avg_pred_probs_test = np.mean(np.array(pred_probs_test_arr), axis=0)

    return avg_model_accuacy, avg_pred_probs_unlabeled, avg_pred_probs_test
...
def train_cross_val_model(
    ...
    test_data,
    unlabeled_data=None,
    model_results_folder="./cross_val_predict_run/",
    *,
    num_cv_folds=5,
    verbose=0,
    epochs=1,
...
def train_test_model(
    ...
    *,
    ...
    ngpus_per_trial=1,
    verbose=0,
    num_cv_folds=2,  # dummy arg to prevent code dup
):
